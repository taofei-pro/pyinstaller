...
set new_hooks_version (curl -L https://pypi.org/pypi/pyinstaller-hooks-contrib/json | jq --raw-output '.releases | keys[-1]')
perl -i -pe 's|pyinstaller-hooks-contrib >= .*|pyinstaller-hooks-contrib >= '$new_hooks_version'|g' setup.cfg
